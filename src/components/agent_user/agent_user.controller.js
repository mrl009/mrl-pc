...
export default function ($scope, $sce, Core, Layer) {
    ...
    $scope.getAgentList = function () {
        Layer.loading()
        Core.get('agent/get_agent_user?username=' + $scope.data.username + '&page=' + $scope.nextAgentIndex, function (json) {
            Layer.closeLoading()
            ...
            if (c.code == 200) {
                ...
                if ($scope.items.length < 15) {
                    $('#agent_user_next').attr('disabled', true)
                } else {
                    $('#agent_user_next').attr('disabled', false)
                }
            }
        }, false)
        if ($scope.nextAgentIndex <= 1) {
            $('#agent_user_pre').attr('disabled', true)
        } else {
            $('#agent_user_pre').attr('disabled', false)
        }
    }
    $scope.getAgentList()
