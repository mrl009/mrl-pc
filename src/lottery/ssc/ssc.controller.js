...
import './ssc.lottery.less'
import $ from 'jQuery'

export default function ($scope, $timeout, $interval, Core, Layer, $stateParams, Lottery, Ssc) {
    ...
    $scope.upDateKithe = function () {
        ...
        $scope.$on('$destroy', function () {
            ...
            //$interval.cancel($scope.openTimer2)
        })
    ...
    $scope.onStart = function () {
        Core.get('games/play/' + $scope.gid, function (json) {
            ...
            if (c.code == 200) {
                ...
                $scope.getPros()//获取球号
                if ($scope.sessionBet.length) {
                    const t = $scope.playlist.play[0]
                    $timeout(function () {
                        $scope.getPlay(t.id, t.name)
                    }, 500)
                }
                $timeout(function () {
                    Layer.closeLoading()
    ...
    //点击顶级玩法
    $scope.p2 = []
    $scope.getPlay = function (pid, ptitle) {
    ...
    $scope.getBall = function (id) {
        ...
        $scope.getPinfo(id)
        $scope.getTips()//获取提示
        if ($scope.playInfo.sname == 'zxhz') {
            $scope.tmp = 'ssc/hz.tpl.html'
        } else if ($scope.playInfo.sname == 'zx' || ($scope.playInfo.cname == 'rx3' && $scope.playInfo.sname == 'z3' || $scope.playInfo.sname == 'z6')) {
            $scope.tmp = 'ssc/zx.tpl.html'
        } else {
            $scope.tmp = 'ssc/balls.tpl.html'
        }
    }
    ...
    $scope.editRate = function () {
        $scope.$watch('rate.volume', function () {
            ...
            if ($scope.rate != undefined && $scope.rate.rate != undefined) {
                arr = $scope.rate.rate.split(',')
                arrmin = $scope.rate.rate_min.split(',')
            }
            $scope.orate = []
            if ($scope.playInfo.aname == 'lh') {
                let arr = $scope.lottery.balls[0].child
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    $scope.orate[i] = Core.cutZero($scope.orate[i].toFixed(3))
                }
            } else {
                arr = $scope.rate.rate.split(',')
                arrmin = $scope.rate.rate_min.split(',')
    ...
    $scope.selBalls = function (event) {
        $scope.suData.sumbet = Ssc.selBalls(event.target, $scope.playInfo)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //工具选择
    $scope.toolXz = function (event, type) {
        $scope.suData.sumbet = Ssc.toolXz(event.target, $scope.playInfo, type)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //清除选中及金额
    ...
    $scope.addBet = function () {
        ...
        $scope.betList = Ssc.addBet($scope, $scope.betList)
        $scope.countBet()
        $scope.clearBet() //清理数据
    }
    //统计总注数及金额
    $scope.betSum = 0 //下注总数
    $scope.betTotal = 0 //下注总金额
    ...
    //开启是否追号
    $scope.bb = {zhbet: false, type: false} //追号---------- type 0中奖不停1中奖后停止
    //提交到下注
    ...
    /*************随机一注*****************/
    $scope.oneBet = function () {
        let sel = Ssc.oneBet($scope.playInfo)
        angular.forEach(sel, function (d) {
            $scope.selBalls(d)
        })
        $scope.addBet()
    }
    /*************随机五注*****************/
    $scope.fiveBet = function () {
        for (let i = 1; i <= 5; i++) {
            $scope.oneBet()
        }
    }
    ...
    if ($scope.sessionBet.length > 0) {
        $timeout(function () {
            ...
            if ($scope.sessionBet.gid == $scope.gid && $scope.sessionBet.type == $scope.type) {
                $scope.suData.txtmoney = $scope.sessionBet.multiple*2
                let sel = Ssc.setSel($scope.sessionBet.number)
                angular.forEach(sel, function (d) {
                    $scope.selBalls(d)
                })
