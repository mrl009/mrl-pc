...
export default function ($scope, $timeout, $interval, Core, Layer, $stateParams, Lottery, Yb) {
    ...
    //点击顶级玩法
    $scope.p2 = []
    $scope.getPlay = function (pid, ptitle = '三码', psname) {
        ...
        if ($scope.p2.length == 0) {
            $scope.p2 = $scope.playlist.play[0].play
            $scope.showParentId = $scope.playlist.play[0].id
            $scope.getBall($scope.playlist.play[0].play[0].play[0].id)
        }
    }
    //获取球号
    ...
    $scope.selBalls = function (event) {
        $scope.suData.sumbet = Yb.selBalls(event.target, $scope.playInfo)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //工具选择
    $scope.toolXz = function (event, type) {
        $scope.suData.sumbet = Yb.toolXz(event.target, $scope.playInfo, type)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //清除选中及金额
    ...
    $scope.addBet = function () {
        ...
        $scope.betList = Yb.addBet($scope, $scope.betList)
        $scope.countBet()
        $scope.clearBet() //清理数据
    }
    //统计总注数及金额
