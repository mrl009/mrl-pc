...
export default function ($scope, $timeout, $interval, Core, Layer, $stateParams, Lottery, Pk10) {
    ...
    $scope.selBalls = function (event) {
        $scope.suData.sumbet = Pk10.selBalls(event.target, $scope.playInfo)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //工具选择
    $scope.toolXz = function (event, type) {
        $scope.suData.sumbet = Pk10.toolXz(event.target, $scope.playInfo, type)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //清除选中及金额
    ...
    $scope.addBet = function () {
        ...
        $scope.betList = Pk10.addBet($scope, $scope.betList)
        $scope.countBet()
        $scope.clearBet() //清理数据
    }
    //统计总注数及金额
    ...
    if ($scope.sessionBet.length > 0) {
        $timeout(function () {
            ...
            if ($scope.sessionBet.gid == $scope.gid && $scope.sessionBet.type == $scope.type) {
                ...
                let sel = Pk10.setSel($scope.sessionBet.number)
