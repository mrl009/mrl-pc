...
export default function ($scope, $timeout, $interval, Core, Layer, $stateParams, Lottery, $11x5) {
    ...
    $scope.selBalls = function (event) {
        $scope.suData.sumbet = $11x5.selBalls(event.target, $scope.playInfo)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //工具选择
    $scope.toolXz = function (event, type) {
        $scope.suData.sumbet = $11x5.toolXz(event.target, $scope.playInfo, type)
        $scope.suData.money = $scope.suData.sumbet * $scope.suData.txtmoney
    }
    //清除选中及金额
    ...
    $scope.addBet = function () {
        ...
        $scope.betList = $11x5.addBet($scope, $scope.betList)
        $scope.countBet()
        $scope.clearBet() //清理数据
    }
    //统计总注数及金额
